...
# 1. Load the model using a direct relative path for Streamlit Cloud
@st.cache_resource
def load_model():
    try:
...
with col2:
    ...
    loc_options = sorted([loc.replace("Location_", "") for loc in all_features if "Location_" in loc])
...
if st.button("Predict Price", use_container_width=True):
    ...
    # If raw_pred is around 2.5, your model likely predicts log(Price in Crores/Lakhs)
    # Applying expm1 converts log back to the training unit
    final_price = np.expm1(raw_pred)
    
    st.success(f"### Estimated Price: {final_price:,.2f} Lakh")
    st.info(f"Debug Info: Raw Model Output = {raw_pred:.4f}")
